...
int main()
{
    ...
    for (int i = 1; i <= exponent; i++) {
        ...
        double* a_array = new double[n + 1]; fill_n(a_array, n, -1.0);
        double* c_array = new double[n + 1]; fill_n(c_array, n, -1.0);
        double* d_array = new double[n + 1]; fill_n(d_array, n, 2.0);
        double* x = new double[n + 1];
        double* solution = new double[n + 1];
        double* b_array = new double[n + 1];

        //Updating solution with known endpoints.
        solution[0] = solution[n] = 0.0;

        //Fetches current time. Used in determining  execution time.
        auto start = chrono::high_resolution_clock::now();

        //Updates diagonal values (d) and calculates values of b for every x_i.
        for (int i = 2; i < n; i++) {
            d_array[i] = d_array[i] - ((a_array[i] * c_array[i - 1]) / d_array[i - 1]);
        }
        for (int i = 0; i <= n; i++) {
            x[i] = i * h;
            b_array[i] = hh * f(i * h);
